...
int dx[4] = {-1,0,1,0};
int dy[4] = {0,-1,0,1};
bool in_range(int y, int x, int H, int W) { return 0<=y&&y<H&&0<=x&&x<W; }
...
bool operator == (const Data& a, const Data& b) { return equals(a.y, b.y) && equals(b.x, b.x) && a.w == b.w; }
...
pair<int, int> getBlockBottomOnBlock(int mycol, int pcol) {
  ...
  int top = -1;
  rep(i, H) rep(j, W) {
    if(color[i][j] != -1 && color[i][j] == mycol && in_range(i+1, j, H, W) && color[i+1][j] == pcol) {
      if(top > i) {
        top = i;
        xL = j;
        xR = j+1;
...
Data rec(int col) {
  ...
  for(auto u: vc) {
    ...
    if(e.x <= xlxr.first || xlxr.second <= e.x) return InvalidData;
  }

  if(w > 0) {
    auto xlxr = getBlockHeadX(col);
    double cy = y / w, cx = x / w;
    if(!safeY(cy, col)) return InvalidData; // topが高いほうが下
...
bool solve() {
  ...
  for(int j=W-1; j>=0; j--) if(color[H-1][j] != -1) {
    xr = j+1;
    break;
  }
